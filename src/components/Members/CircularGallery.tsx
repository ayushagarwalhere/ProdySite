...
export default function CircularGallery({
  ...
  scrollSpeed = 2,
  scrollEase = 0.05,
  autoPlaySpeed = 0.2 // Default auto-play speed
  ...
  useEffect(() => {
    const handleUpdate = () => {
      if (appRef.current && !appRef.current.isDown) {
        appRef.current.scroll.target += autoPlaySpeed;
      }
      requestRef.current = requestAnimationFrame(handleUpdate);
    };

    const requestRef = { current: requestAnimationFrame(handleUpdate) };
