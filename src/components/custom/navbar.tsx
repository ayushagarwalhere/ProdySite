    ...
    <>
      ...
      <nav style={{
        ...
        {scrolled && (
          <div style={{
            ...
            animation: "scanLine 3s ease .2s both", pointerEvents: "none",
          }} />
        )}

        {/* logo */}
        <Link href="/" style={{ display: "flex", alignItems: "center", gap: 10, textDecoration: "none" }}>
          <svg width="22" height="16" viewBox="0 0 22 16" fill="none" style={{ flexShrink: 0, opacity: .8 }}>
            ...
            <path d="M11 11.5 Q8.5 15 11 17 Q13.5 15 11 11.5Z" stroke={GOLD} strokeWidth="0.6" fill="none" opacity="0.6" />
          </svg>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          ...
          >
            {[
              ...
              { w: 16, tf: isOpen ? "translateX(20px)" : "none", op: isOpen ? 0 : 1 },
              { w: 24, tf: isOpen ? "translateY(-7px) rotate(-45deg)" : "none" },
            ].map(({ w, tf, op }, i) => (
              <span key={i} style={{
                display: "block", width: w, height: "1.5px", background: GOLD,
                transition: "all .35s cubic-bezier(.22,1,.36,1)",
                transformOrigin: "center", transform: tf, opacity: op ?? 1,
              }} />
            ))}
      ...
      <div style={{
        ...
        <div onClick={() => setIsOpen(false)} style={{
          position: "absolute", inset: 0, background: "rgba(4,3,2,.75)",
        ...
        <div style={{
          ...
          {/* ambient glows */}
          <div style={{ position: "absolute", width: 300, height: 300, top: "10%", right: "-80px", borderRadius: "50%", background: "radial-gradient(circle,rgba(180,124,60,.07) 0%,transparent 65%)", pointerEvents: "none" }} />
          <div style={{ position: "absolute", width: 200, height: 200, bottom: "15%", left: "-40px", borderRadius: "50%", background: "radial-gradient(circle,rgba(140,80,20,.06) 0%,transparent 65%)", pointerEvents: "none" }} />
          ...
          <div style={{
            ...
            <svg width="18" height="14" viewBox="0 0 18 14" fill="none">
              <path d="M0.5 7 Q9 0.5 17.5 7 Q9 13.5 0.5 7Z" stroke={GOLD} strokeWidth="0.8" fill="none" opacity="0.5" />
              <circle cx="9" cy="7" r="2.5" stroke={GOLD} strokeWidth="0.7" fill="none" opacity="0.5" />
              <circle cx="9" cy="7" r="1" fill={GOLD} opacity="0.5" />
            </svg>
            <span style={{ fontFamily: "'Syne',sans-serif", fontSize: 9, letterSpacing: ".28em", textTransform: "uppercase", color: GOLD, fontWeight: 700, opacity: .5 }}>
              Navigation
            </span>
          </div>
            ...
            {navLinks.map((link, i) => (
              <Link
                ...
                style={{
                  ...
                  transition: `opacity .4s ease ${.12 + i * .07}s, transform .4s cubic-bezier(.22,1,.36,1) ${.12 + i * .07}s, border-color .25s`,
                }}
              >
                <span className="menu-link-num">{link.num}</span>
                <span className="menu-link-text">{link.label}</span>
                <svg className="menu-link-arrow" width="16" height="10" viewBox="0 0 16 10" fill="none">
                  <path d="M1 5h12M8 1l4 4-4 4" stroke={GOLD} strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </Link>
            ))}
          </div>
          ...
          <div style={{
            ...
            opacity: isOpen ? 1 : 0, transition: "opacity .4s ease .48s",
          }}>
          ...
          <div style={{
            ...
            <Link href="/login" onClick={() => setIsOpen(false)} className="auth-btn-ghost">Login</Link>
            <Link href="/signup" onClick={() => setIsOpen(false)} className="auth-btn-solid">Sign Up</Link>
          </div>

          {/* bottom cartouche */}
          <div style={{
            position: "absolute", bottom: "2.5rem", left: "3rem", right: "3rem",
            opacity: isOpen ? .22 : 0, transition: "opacity .4s ease .6s",
          }}>
            <svg width="100%" height="14" viewBox="0 0 300 14" preserveAspectRatio="xMidYMid slice" style={{ display: "block" }}>
              {Array.from({ length: 16 }).map((_, i) => (
                <g key={i} transform={`translate(${i * 20 - 5},0)`}>
                  <rect x="1" y="4" width="18" height="6" rx="1" fill="none" stroke={GOLD} strokeWidth="0.6" />
